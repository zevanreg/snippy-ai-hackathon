...
// Zero Trust toggles
// ==========================
@description('Enable Zero Trust private networking for all services')
param zeroTrustEnabled bool = true
...
param privateDnsZoneBlob string = 'privatelink.blob.' + environment().suffixes.storage

@description('Private DNS zone name for Key Vault')
param privateDnsZoneKeyVault string = replace('privatelink.' + environment().suffixes.keyvaultDns, 'vault.azure.net', 'vaultcore.azure.net')

// Zero Trust networking module (VNet, private endpoints, DNS)
module zeroTrust 'app/network-zero-trust.bicep' = if (zeroTrustEnabled) {
  name: 'zeroTrust'
  scope: rg
  params: {
    location: location
    ...
    keyVaultId: aiProject.outputs.keyVaultId
    privateDnsZoneDocuments: privateDnsZoneDocuments
    privateDnsZoneBlob: privateDnsZoneBlob
    privateDnsZoneKeyVault: privateDnsZoneKeyVault
  }
}
...
module storage 'br/public:avm/res/storage/storage-account:0.8.3' = {
  ...
  params: {
    ...
    publicNetworkAccess: zeroTrustEnabled ? 'Disabled' : 'Enabled'
  networkAcls: zeroTrustEnabled ? {
      bypass: 'None'
      defaultAction: 'Deny'
      virtualNetworkRules: [
        {
      virtualNetworkResourceId: zeroTrust.outputs.functionsSubnetId
        }
      ]
    } : {
      targetScope = 'subscription'
